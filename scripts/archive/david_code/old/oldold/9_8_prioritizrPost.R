...
plaything <- habitatProportion %>% 
  st_drop_geometry() %>% 
  left_join(bmShapes, by = "id") %>% 
  st_as_sf()



maxFWBLM <- problem(plaything, c("fox"), cost_column = "revenue") %>% 
  add_max_features_objective(cost) %>% 
  add_absolute_targets(c(foxTarget)) %>% 
  add_boundary_penalties(penalty = 0.1, edge_factor = 0, 
                         data = boundary_matrix(plaything))
...
eval_cost_summary(maxFWBLM, maxFWBSolve %>% dplyr::select(solution_1))

eval_n_summary(maxFWBLM, maxFWBSolve %>% dplyr::select(solution_1))
eval_target_coverage_summary(maxFWBLM, maxFWBSolve %>% dplyr::select(solution_1))
...
runs <- list(minFW, maxFW, minF, maxF, minW, maxW, minFWJ,
             maxFWJ, minFJ, maxFJ, minWJ, maxWJ, maxFWBLM)

runNames <- c("minFW, maxFW, minF, maxF, minW, maxW, minFWJ,
             maxFWJ, minFJ, maxFJ, minWJ, maxWJ, maxFWB") %>% 
  str_split(",[:space:]*") %>% 
  unlist()
