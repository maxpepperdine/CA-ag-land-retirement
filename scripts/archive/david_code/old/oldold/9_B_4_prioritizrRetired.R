...
# Convert COMM function ---------------------------------------------------


convertFunc <- function(problem) {
  ...
  problem %>%
    ...
    mutate(
      ...
      Crop = case_when(
        ((locked_out == 1) & (solution_1 == 1)) ~ "UNCULTIVATED",
        ((locked_out == 0) & (solution_1 == 1)) ~ "FALLOW",
        TRUE ~ COMM
      ),
      
      
      fal = case_when(
        Crop == "UNCULTIVATED" ~ "UNCULTIVATED",
        Crop == "FALLOW" ~ "FALLOW",
        T ~ "NA"
      )
    )
...
newProblem1 <- problem(habitatProportion, c("fox", "water"), cost_column = "revenue") %>%
  add_min_set_objective() %>%
...
vector <- newProblem1 %>% 
  ...
  pivot_longer(everything(), names_to = "Targets", values_to = "val") %>% 
  pull(val)


newProblem2 <- newProblem1 %>%
  problem(c("fox", "water"), cost_column = "revenue") %>%
  add_min_set_objective() %>%
  add_absolute_targets((c(foxTarget, waterTarget)) - vector) %>%
  add_locked_out_constraints("locked_out") %>% 
  convertFunc()



solveOutput(minFW, newProblem2)
...
newProblem2%>%
  filter(fal != "NA") %>%
  tm_shape() +
  tm_polygons(col = "fal", palette = "viridis") +
  tm_shape(habitatProportion) +
  tm_fill(alpha = 0.5)
...
whatever %>%
  ...
  pivot_wider(names_from = fal, values_from = area) %>% 
  mutate(
    rat = UNCULTIVATED / (UNCULTIVATED + FALLOW)
  )
