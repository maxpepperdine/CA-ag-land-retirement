...
whatever <- solShp2 %>%
  mutate(
    Crop = case_when(((locked_out == 1) & (solution_1 == 1)) ~ "UNCULTIVATED",
                     ((locked_out == 0) & (solution_1 == 1)) ~ "FALLOW",
                     TRUE ~ COMM),
    fal = case_when(Crop == "UNCULTIVATED" ~ "UNCULTIVATED",
                    Crop == "FALLOW" ~ "FALLOW",
                    T ~ "NA")
  )
...
minFW1 <- problem(habitatProportion, c("fox", "water"), cost_column = "revenue") %>% 
  add_min_set_objective() %>% 
...
solF$falType[sample(x = nrow(solF), size = round(nrow(solF) * retTarget))] <- 1
...
solveOutput(minFW1, solve(minFW1))
