...
# Weight admin /support jobs over crop sectors

# Find total jobs for ag sectors 
cropTotal <- singleFile %>% 
  filter(str_detect(industry_code, "^.....$")) %>% 
  {{sum(.$jobs)}}

# Find total jobs for ag supportsectors 
supportTotal <- singleFile %>% 
  filter(str_detect(industry_code, "^......$")) %>% 
  {{sum(.$jobs)}}


# Add weighted support jobs to labor
jobWeighted <- singleFile %>% 
  filter(str_detect(industry_code, "^.....$")) %>% 
  mutate(
    cropProp = jobs / cropTotal,
    supportAdd = cropProp * supportTotal,
    jobsAdjusted = (supportAdd + jobs) %>% round()
  ) %>% 
  mutate(industry = str_trim(industry)) %>%
  # Using multiple job holders equation from oil and agriculture report, 
  # convert jobs to workers
  mutate(
    mjh = round((jobsAdjusted * 0.035) - (3 * (jobsAdjusted * 0.003))) ,
    workers = (jobsAdjusted - mjh) %>% round()
  ) %>% 
  select(-mjh)


write_csv(jobWeighted, "./Data/0_input/jobsPerSector.csv")
sum(jobWeighted$jobsAdjusted)
sum(jobWeighted$workers)
