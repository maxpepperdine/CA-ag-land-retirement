...
freqMapRaw <- kernNew %>% 
  select(id, selected) %>%   
  rasterize(rast_template, field = "selected", 
            #touches = TRUE, 
            fun = mean) %>% 
  as.polygons() %>% 
  st_as_sf() %>% 
  #filter(selected != 0) %>% 
  mutate(
    `Frequency` = case_when(#selected == 0 ~ "0 None",
                             selected < 3 ~ "0-2 Low", 
                             selected < 5 ~ "3-4 Medium",
                             T ~ '5-6 High'),
    `Frequency` = factor(`Frequency`, levels = c("0-2 Low", "3-4 Medium", "5-6 High"))
  ) #%>% 
