# =============================================================================
# PRIORITIZR COMBINED OPTIMIZATION (APPLIED WATER) — RESULTS VISUALIZATIONS
# =============================================================================
# Purpose: Generate figures from the AW-based combined water + habitat 
#          prioritizr results produced by 9_3_prioritizr_combined_AW.R.
#
# This is the AW-feature counterpart to 10_3_prioritizr_combined_figures.R,
# providing a parallel visual analysis using applied water (AW) as the water 
# feature rather than S_net consumption.
#
# FIGURES:
#   1. Scenario comparison grid: Acres, Revenue, AW saved across 6 scenarios
#   2. Target achievement heatmap: 16 features × 6 scenarios
#   3. Spatial maps: HQ across 3 water scenarios
#   3b. 4-panel spatial maps: 3 scenarios + selection frequency
#   4. Selection frequency across water scenarios (Suit + HQ)
#   5. Cost premium of dual objectives: combined vs. water-only vs. habitat-only
#
# INPUTS:
#   - prioritizr_combined_AW_results.RData (from 9_3_prioritizr_combined_AW.R)
#   - prioritizr_cross_temporal_results.RData (from 9_1, for cost premium fig)
#   - valley_wide_summary.csv (from 9_2_prioritizr_water_only_AW, for cost premium fig)
#
# OUTPUTS:
...
# Output directory
fig_dir <- here("data/intermediate/FallowFoxes_SJV_archive/10_3_prioritizr_combined_figures_AW/")
...
# --- Combined AW results ---
load(here("data/intermediate/FallowFoxes_SJV_archive/9_3_prioritizr_combined_AW/prioritizr_combined_AW_results.RData"))
...
# Pulls from the AW water-only analysis to keep the comparison methodologically consistent
...
# --- Diagnostics ---
cat("Loaded combined AW prioritizr results:\n")
...
fig1 <- fig1a + fig1b + fig1c +
  ...
  plot_annotation(
    title = "Combined optimization results across climate scenarios (applied water)",
...
fig2 <- ggplot(heatmap_data, aes(x = scenario_label, y = feature_label, fill = pct_achieved)) +
  ...
  labs(
    title = "Target achievement across all combined scenarios (applied water)",
...
fig3 <- map_base + map_rcp45 + map_rcp85 +
  plot_layout(ncol = 3, guides = "collect") +
  plot_annotation(
    title = "Optimal retirement configuration across water scenarios (combined AW, high quality habitat)",
...
# --- Bounding box (with larger buffer for 4-panel layout) ---
field_bbox <- st_bbox(field_data)
...
fig3b_titled <- ggdraw() +
  ...
  draw_label("Spatial distribution of fields selected for retirement (combined AW + habitat, high quality)",
...
# --- Build the two maps ---
make_freq_map <- function(data, title_text) {
  ggplot() +
    geom_sf(data = sjv_basins, fill = NA, color = "gray60", linewidth = 0.4) +
    geom_sf(data = data, aes(fill = n_selected_fct),
            color = NA, linewidth = 0) +
    geom_sf(data = sjv_basins, fill = NA, color = "gray40", linewidth = 0.3) +
    coord_sf(xlim = plot_bbox[c("xmin", "xmax")],
             ylim = plot_bbox[c("ymin", "ymax")],
...
fig4_suit <- make_freq_map(freq_suit, "A: Suitable habitat")
fig4_hq   <- make_freq_map(freq_hq,   "B: High Quality habitat")

fig4 <- fig4_suit + fig4_hq +
  plot_layout(ncol = 2, guides = "collect") +
  plot_annotation(
    title = "Selection frequency across water scenarios (combined AW + habitat)",
    subtitle = "Fields selected in all 3 water scenarios are no-regret retirements",
    theme = theme(
      plot.title    = element_text(face = "bold", size = 14),
      plot.subtitle = element_text(color = "gray40", size = 10)
    )
...
# =============================================================================
# Compares foregone revenue and acres retired for:
#   - Water only (from 9_2_prioritizr_water_only_AW)
#   - Habitat only (from 9_1_prioritizr_habitat_only; skipped if unavailable)
#   - Combined (current analysis)
# Split by water scenario (Baseline, RCP 4.5) for focused comparison.
...
comparison <- tibble(
  analysis    = character(),
  water_label = character(),
  quality     = character(),
  total_cost  = numeric(),
  total_acres = numeric()
)

# Combined (all 6 scenarios)
comparison <- comparison %>%
  bind_rows(
    summary_all %>%
      transmute(
        analysis    = "Combined",
        water_label = water_scenario,
        quality     = quality,
...
# Water only (from 9_2 AW)
if (!is.null(water_summary)) {
  comparison <- comparison %>%
    bind_rows(
      water_summary %>%
        mutate(
          water_label = case_when(
        ...
        transmute(
          ...
          total_cost  = revenue_cost_usd,
          total_acres = acres_selected
        )
    )
}

# Habitat only (if available)
if (!is.null(habitat_results) && "summary_all" %in% ls(habitat_results)) {
  hab_summary_cross_temporal <- habitat_results$summary_all %>%
    filter(scenario_name == "cross_temporal")
  
  if (nrow(hab_summary_cross_temporal) > 0) {
    comparison <- comparison %>%
      bind_rows(
        hab_summary_cross_temporal %>%
          transmute(
            analysis    = "Habitat only",
            water_label = "Baseline",  # Habitat-only isn't scenario-specific
            quality     = quality,
            total_cost  = total_cost,
            total_acres = total_acres
          )
...
# --- Build fig5 focused on Baseline and RCP 4.5 ---

fig5_data <- comparison %>%
  filter(water_label %in% c("Baseline", "RCP45") | analysis == "Habitat only") %>%
  mutate(
    bar_label = case_when(
      analysis == "Water only"   & water_label == "Baseline" ~ "Water only\n(Baseline)",
      analysis == "Water only"   & water_label == "RCP45"    ~ "Water only\n(RCP 4.5)",
      analysis == "Habitat only" & quality     == "Suitable" ~ "Habitat only\n(Suitable)",
      analysis == "Habitat only" & quality     == "High Quality" ~ "Habitat only\n(High Quality)",
      analysis == "Combined"     & water_label == "Baseline" & quality == "Suitable"     ~ "Combined\n(Baseline, Suit)",
      analysis == "Combined"     & water_label == "Baseline" & quality == "High Quality" ~ "Combined\n(Baseline, HQ)",
      analysis == "Combined"     & water_label == "RCP45"    & quality == "Suitable"     ~ "Combined\n(RCP 4.5, Suit)",
      analysis == "Combined"     & water_label == "RCP45"    & quality == "High Quality" ~ "Combined\n(RCP 4.5, HQ)"
    ),
    bar_fill = case_when(
      analysis == "Water only"                                     ~ "#3266ad",
      analysis == "Habitat only" & quality == "Suitable"           ~ "#8FBC8F",
      analysis == "Habitat only" & quality == "High Quality"       ~ "#2E7D32",
...
# --- Helper functions to build cost/acres panels ---
...
fig5 <- (fig5a + fig5b) / (fig5c + fig5d) +
  plot_annotation(
    title = "Cost of co-optimization: combined vs. individual objective solutions (applied water)",
    subtitle = "Comparing AW water-only, habitat-only, and combined optimization under Baseline and RCP 4.5 scenarios",
    theme = theme(
      plot.title    = element_text(face = "bold", size = 13),
      plot.subtitle = element_text(color = "gray40", size = 10)
    )
...
fig5_full <- ggplot(fig5_full_data, aes(x = water_label_clean, y = total_cost / 1e6,
                                        fill = bar_group)) +
  ...
  labs(
    ...
    subtitle = "Combined optimization costs vs. AW water-only baseline for each climate scenario",
...
ggsave(file.path(fig_dir, "fig5b_cost_premium_all_scenarios.png"), fig5_full,
...
kbl_overall <- overall_table %>%
  kbl(
    caption = "Combined optimization results: applied water savings + habitat creation",
    align = c("l", "l", rep("r", 5))
  ) %>%
  kable_styling(
  ...
  footnote(
    general = paste0("All scenarios meet 25,000-acre habitat targets for BNLL, GKR, and SJKF ",
                     ...
                     "PPIC's 6.3% climate adjustment + BCMv8 PET ratio. BLM = ", chosen_blm, "."),
    general_title = "Note: "
  )

save_kable(kbl_overall, file.path(fig_dir, "table_combined_AW_summary.html"))
cat("  Saved: table_combined_AW_summary.html\n")

write_csv(overall_table, file.path(fig_dir, "table_combined_AW_summary.csv"))
cat("  Saved: table_combined_AW_summary.csv\n")


cat("\n========== ALL FIGURES GENERATED (AW combined) ==========\n")
