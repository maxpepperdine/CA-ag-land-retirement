...
# Scraped Data from AIC Labor - Hours Budget Estimates 

# Crop corresponds to AIC Crop categories
...
# Combined labor hour value per acre for production
...
# For Crop types with multiple studies done in the 2010- 2020 period, 
# find mean labor hour value per acre
...
budget <- newDat %>% 
  ...
  # First find mean value for each crop - year group
  group_by(Crop, Year) %>% 
  mutate(
    Labor2 = mean(Labor) %>% round(2)
  ) %>%
  # Take average of each years' mean
  group_by(Crop) %>% 
  mutate(
...
write_csv(budget, "./Data/1_cropRevenueJobsCrosswalk/jobsBudget.csv")
...
# Build Crosswalk to join AIC labor hours data to KCACR Value Data

# Crop is KCACR crop type
...
# Match is matching crop from AIC
...
# Build crosswalk to join QCEW jobs data to KCACR

# crop1 is KCACR Crop types
...
write_csv(finalTable, "./Data/1_cropRevenueJobsCrosswalk/cropRevJobsFinal.csv")
