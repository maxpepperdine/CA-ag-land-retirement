...
# # use terra::subst() to reclassify mukey raster to soil properties
# 
# # create lookup data frames for each property
# # subst() requires 'from' and 'to' columns
# lut_clay <- data.frame(from = lookup$mukey, to = lookup$clay)
# lut_ph <- data.frame(from = lookup$mukey, to = lookup$ph)
# lut_ec <- data.frame(from = lookup$mukey, to = lookup$ec)
# 
# 
# # apply subst() for each property
...
# Create named vectors for fast, reliable lookup
# Names are mukeys (as character), values are soil properties
clay_lookup <- setNames(lookup$clay, as.character(lookup$mukey))
ph_lookup <- setNames(lookup$ph, as.character(lookup$mukey))
ec_lookup <- setNames(lookup$ec, as.character(lookup$mukey))

# Extract mukey values from raster
mukey_vals <- values(mukey_sjv)

# Perform lookup using named vector indexing
# Converting to character ensures type matching
clay_vals <- clay_lookup[as.character(mukey_vals)]
ph_vals <- ph_lookup[as.character(mukey_vals)]
ec_vals <- ec_lookup[as.character(mukey_vals)]

# Create new rasters with the looked-up values
clay_rast <- setValues(rast(mukey_sjv), as.numeric(clay_vals))
ph_rast <- setValues(rast(mukey_sjv), as.numeric(ph_vals))
ec_rast <- setValues(rast(mukey_sjv), as.numeric(ec_vals))
...
# Verify the reclassification worked correctly
cat("Verification of reclassified rasters:\n")
cat("  Clay range:", round(min(values(clay_rast), na.rm = TRUE), 2), "-", 
    round(max(values(clay_rast), na.rm = TRUE), 2), "%\n")
cat("  pH range:", round(min(values(ph_rast), na.rm = TRUE), 2), "-", 
    round(max(values(ph_rast), na.rm = TRUE), 2), "\n")
cat("  EC range:", round(min(values(ec_rast), na.rm = TRUE), 2), "-", 
    round(max(values(ec_rast), na.rm = TRUE), 2), "dS/m\n")
