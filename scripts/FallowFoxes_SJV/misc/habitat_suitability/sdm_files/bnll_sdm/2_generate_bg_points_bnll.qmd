...
bnll_occs <- read_csv(here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/1_extract_occ_env_data_bnll/bnll_occ_env_data.csv"))
```
...
bnll_buffer <- st_buffer(bnll_sf, dist = 50000)
...
# buffer is too long for rgbif, so simplify it (nchar ~ 1500 is ideal)
...
write_csv(background_df, here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/2_generate_bg_points_bnll/bnll_bg_points.csv"))
```
