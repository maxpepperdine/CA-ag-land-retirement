...
This script generates plots and difference maps comparing the baseline BNLL habitat suitability model with future predictions under various emissions scenarios. 
...
sjv_boundary <- st_transform(sjv_boundary, crs = crs(bnll_baseline))
crs(sjv_boundary)
# convert to terra vector
sjv_vect <- vect(sjv_boundary)

# transform CA boundary to EPSG:3310
ca_boundary <- st_transform(ca_boundary, crs = crs(bnll_baseline))
crs(ca_boundary)
# convert to terra vector
ca_vect <- vect(ca_boundary)

# transform basins
sjv_basins <- st_transform(sjv_basins, crs = crs(bnll_baseline))
crs(sjv_basins)
# convert to terra vector
...
Calculated in `4_maxent_predictions_bnll.qmd`, reclassify continuous habitat suitability layers into binary suitable/unsuitable layers using the threshold that maximizes true positive/negative rate (i.e., specificity + sensitivity). This is **0.21** for the BNLL maxent model.

```{r}
# define the reclass matrix
reclass_matrix_future <- matrix(c(0, 0.21, 0,
                                  0.21, 1, 1),
                                ncol = 3, byrow = TRUE)
...
combined_rcp45_2049 <- bnll_baseline_bin * 10 + bnll_rcp45_2049_bin
combined_rcp45_2069 <- bnll_baseline_bin * 10 + bnll_rcp45_2069_bin
combined_rcp85_2049 <- bnll_baseline_bin * 10 + bnll_rcp85_2049_bin
combined_rcp85_2069 <- bnll_baseline_bin * 10 + bnll_rcp85_2069_bin
...
main_map_cont <- tm_shape(cont_diff_stack, bbox = basin_bbox_expanded) +
...
```{r}
# clip the cont future predictions to basin boundary
bnll_rcp45_2049_basin <- bnll_rcp45_2049 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
bnll_rcp45_2069_basin <- bnll_rcp45_2069 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
bnll_rcp85_2049_basin <- bnll_rcp85_2049 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
bnll_rcp85_2069_basin <- bnll_rcp85_2069 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)

# stack the future predictions and name them
future_stack <- c(bnll_rcp45_2049_basin, bnll_rcp45_2069_basin, 
                  bnll_rcp85_2049_basin, bnll_rcp85_2069_basin)
names(future_stack) <- c("RCP 4.5 (2020-2049)", "RCP 4.5 (2040-2069)", 
                         "RCP 8.5 (2020-2049)", "RCP 8.5 (2040-2069)")

# plot the future predictions
bnll_fut_map <- tm_shape(future_stack, bbox = basin_bbox_expanded) +
...
bnll_baseline_basin <- bnll_baseline %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)

# plot baseline predictions
bnll_baseline_map <- tm_shape(bnll_baseline_basin, bbox = basin_bbox_expanded) +
