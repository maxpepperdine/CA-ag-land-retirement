---
title: "BNLL MaxEnt prediction generation"
...
This script reads in the prepared BNLL occurrence records (produced in `1_extract_occ_env_data_bnll.qmd`) and background points (produced in `2_generate_bg_points_bnll.qmd` and `3_extract_bg_env_data_bnll.qmd`) to evaluate and generate MaxEnt models using the `dismo` package.
...
occ_data <- read_csv(here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/1_extract_occ_env_data_bnll/bnll_occ_env_data.csv"))

# load background data with env data extracted
bg_data <- read_csv(here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/3_extract_bg_env_data_bnll/bnll_bg_env_data.csv"))
```
...
occ_data <- occ_data %>%
  dplyr::select(-c(cwd1991_2020_ave, pet1991_2020_ave, sand_pct_0_10cm, silt_pct_0_10cm, 
...
max.eval.bnll <- ENMeval::ENMevaluate(occs = occ_data, 
                                      bg = bg_data, 
                                      tune.args = list(fc = c("L", "LQ", "Q"), 
                                                      rm = seq(0.5, 3, 0.5)),
                                      algorithm = "maxnet", 
                                      partitions = "block")
```
...
max.eval.bnll
eval.results(max.eval.bnll) # model evaluation results
eval.tune.settings(max.eval.bnll) # extract the tuning parameter settings 
eval.results.partitions(max.eval.bnll) # assess model performance for each partition

# clean results
results <- eval.results(max.eval.bnll) %>%
  dplyr::select(fc, rm,
                # higher better; demonstrates discriminatory performance on withheld data
                auc.val.avg, 
                # we want lower; models w/ low AUC diff likely transfer better
                auc.diff.avg, 
                # justify model selection; rewards fit to current data
                delta.AICc, 
                # good model should have a value close to 0.1
                or.10p.avg, 
...
kable(results, row.names = FALSE) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE) %>%
  row_spec(3, bold = TRUE)  # highlight the top model based on AIC


# plot stat results across values of the tuning parameters
evalplot.stats(e = max.eval.bnll, 
               # or.mtp me
...
max.eval.filtered <- max.eval.bnll
max.eval.filtered@results <- max.eval.bnll@results[, !colnames(max.eval.bnll@results) %in% "delta.AICc"]
...
save(max.eval.bnll, 
     file = here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/max_eval_bnll.rda"))

# load the evaluation results
load(here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/max_eval_bnll.rda"))
```

## Running MaxEnt & generating predictions

After identifying the best model based on the evaluation results, we'll run the model using the `dismo::maxent()` function. This generates a model object that can be used to predict baseline (historical) and future BNLL occurrence probability under projected environmental data using the `dismo::predict()` function.
...
These model objects are then used to predict baseline (historical) and future BNLL occurrence probabilities.
...
path <- here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/")


## final model run
maxent_bnll <- maxent(x = x, 
                      p = p, 
                      args = args, 
                      path = path)
...
save(maxent_bnll, 
     file = here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/maxent_prediction_bnll_sdm.rda"))
```

#### Predict baseline (historical) BNLL occurrence probability

```{r}
#### set up the prediction parameters

## load the model object
load(here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/maxent_prediction_bnll_sdm.rda"))
...
layers_to_drop <- c("cwd1991_2020_ave", "pet1991_2020_ave", "sand_pct_0_10cm", "silt_pct_0_10cm", 
                    "tmn1991_2020_ave", "tmx1991_2020_ave")
...
filename_bnll <- here::here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/maxent_bnll_pred.tif")
...
dismo::predict(object = maxent_bnll, 
               ...
               filename = filename_bnll, 
               overwrite = TRUE)

## plot the prediction raster!!
pred_rast_bnll <- rast(filename_bnll)
plot(pred_rast_bnll, 
     main = "MaxEnt Predicted Habitat Suitability for Blunt-nosed Leopard Lizard (Gambelia sila)")
```
...
writeRaster(bnll_habitat_masked, 
            here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/maxent_bnll_pred_masked_lakes.tif"),
            overwrite = TRUE)
```

## Predict future BNLL occurrence probability under projected environmental data

Transfer the model to future environmental data under `RCP4.5` and `RCP8.5` for two time periods (`2020-2049` and `2040-2069`) to predict future BNLL occurrence probability under projected environmental conditions.

Produce 4 future prediction rasters: one for each RCP and time period combination.
...
env_fut_folder <- here("data/intermediate/misc/habitat_suitability/env_predictors/future/maxent_ready/bnll/rcp45_2020_2049")
...
filename_bnll_rcp45_2020_2049 <- here::here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/future/maxent_bnll_pred_rcp45_2020_2049.tif")
...
writeRaster(bnll_habitat_masked_rcp45_2020_2049, 
            here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/future/maxent_bnll_pred_rcp45_2020_2049_masked_lakes.tif"),
            overwrite = TRUE)
```
...
env_fut_folder <- here("data/intermediate/misc/habitat_suitability/env_predictors/future/maxent_ready/bnll/rcp45_2040_2069")
...
## define the filename path for the future prediction raster
filename_bnll_rcp45_2040_2069 <- here::here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/future/maxent_bnll_pred_rcp45_2040_2069.tif")

## predict future HEC under RCP4.5 and 2040-2069
dismo::predict(object = maxent_bnll,
               x = x_fut_stack,
               ext = stewart_bnll_ext,
               filename = filename_bnll_rcp45_2040_2069,
               overwrite = TRUE)

## mask lakes from the future prediction raster

# load the future prediction raster
pred_rast_bnll_rcp45_2040_2069 <- rast(filename_bnll_rcp45_2040_2069)

# set lake pixels to 0
bnll_habitat_masked_rcp45_2040_2069 <- pred_rast_bnll_rcp45_2040_2069
bnll_habitat_masked_rcp45_2040_2069[ca_lakes_rast] <- 0

plot(bnll_habitat_masked_rcp45_2040_2069, 
     main = "BNLL habitat suitability; RCP4.5 (2040-2069)")


# save the masked future prediction raster
writeRaster(bnll_habitat_masked_rcp45_2040_2069, 
            here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/future/maxent_bnll_pred_rcp45_2040_2069_masked_lakes.tif"),
            overwrite = TRUE)
```
...
env_fut_folder <- here("data/intermediate/misc/habitat_suitability/env_predictors/future/maxent_ready/bnll/rcp85_2020_2049")
...
## define the filename path for the future prediction raster
filename_bnll_rcp85_2020_2049 <- here::here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/future/maxent_bnll_pred_rcp85_2020_2049.tif")

## predict future HEC under RCP8.5 and 2020-2049
dismo::predict(object = maxent_bnll,
               x = x_fut_stack,
               ext = stewart_bnll_ext,
               filename = filename_bnll_rcp85_2020_2049,
               overwrite = TRUE)

## mask lakes from the future prediction raster

# load the future prediction raster
pred_rast_bnll_rcp85_2020_2049 <- rast(filename_bnll_rcp85_2020_2049)

# set lake pixels to 0
bnll_habitat_masked_rcp85_2020_2049 <- pred_rast_bnll_rcp85_2020_2049
bnll_habitat_masked_rcp85_2020_2049[ca_lakes_rast] <- 0

plot(bnll_habitat_masked_rcp85_2020_2049, 
     main = "BNLL habitat suitability; RCP8.5 (2020-2049)")


# save the masked future prediction raster
writeRaster(bnll_habitat_masked_rcp85_2020_2049, 
            here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/future/maxent_bnll_pred_rcp85_2020_2049_masked_lakes.tif"),
            overwrite = TRUE)
```
...
env_fut_folder <- here("data/intermediate/misc/habitat_suitability/env_predictors/future/maxent_ready/bnll/rcp85_2040_2069")
...
## define the filename path for the future prediction raster
filename_bnll_rcp85_2040_2069 <- here::here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/future/maxent_bnll_pred_rcp85_2040_2069.tif")

## predict future HEC under RCP8.5 and 2040-2069
dismo::predict(object = maxent_bnll,
               x = x_fut_stack,
               ext = stewart_bnll_ext,
               filename = filename_bnll_rcp85_2040_2069,
               overwrite = TRUE)

## mask lakes from the future prediction raster

# load the future prediction raster
pred_rast_bnll_rcp85_2040_2069 <- rast(filename_bnll_rcp85_2040_2069)

# set lake pixels to 0
bnll_habitat_masked_rcp85_2040_2069 <- pred_rast_bnll_rcp85_2040_2069
bnll_habitat_masked_rcp85_2040_2069[ca_lakes_rast] <- 0

plot(bnll_habitat_masked_rcp85_2040_2069, 
     main = "BNLL habitat suitability; RCP8.5 (2040-2069)")


# save the masked future prediction raster
writeRaster(bnll_habitat_masked_rcp85_2040_2069, 
            here("data/intermediate/misc/habitat_suitability/sdm_files/bnll_sdm/4_maxent_predictions_bnll/future/maxent_bnll_pred_rcp85_2040_2069_masked_lakes.tif"),
            overwrite = TRUE)
```

## Calculate MaxEnt threshold for binary predictions

We will threshold continuous suitability values into suitable and non-suitable habitat using the threshold that maximizes the true positive and true negative rate. In other words, this is the threshold that **maximizes sensitivity and specificity**. This threshold will be applied to the continuous prediction rasters to create binary rasters of suitable vs. non-suitable habitat for BNLL under current and future conditions in `5_prediction_plotting_bnll.qmd`.
...
e <- dismo::evaluate(p = occ_coords,       # presence coordinates
                     ...
                     model = maxent_bnll,  # model object from maxent() function
...
thresh_spec_sens <- thresh$spec_sens   # 0.21
```

**NOTE:** Realized this threshold is accessible/already included in the `../4_maxent_predictions_bnll/maxentResults.csv` file, but doesn't hurt to double check.
