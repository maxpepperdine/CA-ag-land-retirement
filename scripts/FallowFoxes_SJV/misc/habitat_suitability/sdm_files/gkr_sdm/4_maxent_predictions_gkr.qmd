...
kable(results, row.names = FALSE) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE) %>%
  row_spec(5, bold = TRUE)  # highlight the top model
...
This section generates a MaxEnt model using the selected feature class and regularization multiplier from the evaluation results above that produced the highest performing models (`fc.LQ_rm.2.5`).

These model objects are then used to predict baseline (historical) and future GKR occurrence probabilities.

#### Baseline (`fc.LQ_rm.2.5`)
...
args <- c("jackknife=TRUE",      # assess variable importance
          ...
          "betamultiplier=2.5")  # this is the rm coefficient from ENMevaluate()
...
thresh_spec_sens <- thresh$spec_sens   # 0.42
```

**NOTE:** Realized this threshold is accessible/already included in the `../4_maxent_predictions_gkr/maxentResults.csv` file, but doesn't hurt to double check.
