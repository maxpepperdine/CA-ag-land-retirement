...
occ_data <- occ_data %>%
  dplyr::select(-c(clay_pct_0_10cm, silt_pct_0_10cm))
bg_data <- bg_data %>%
  dplyr::select(-c(clay_pct_0_10cm, silt_pct_0_10cm))
...
layers_to_drop <- c("clay_pct_0_10cm", "silt_pct_0_10cm")
...
filename_gkr <- here::here("data/intermediate/misc/habitat_suitability/sdm_files/gkr_sdm/4_maxent_predictions_gkr/maxent_gkr_pred_fillNAs.tif")
...
sjv_boundary <- read_sf(here("data/raw/sjv_counties/sjv_counties.shp")) %>%
  st_transform(crs(gkr_habitat_masked))

gkr_habitat_masked_sjv <- gkr_habitat_masked %>%
  mask(vect(sjv_boundary)) %>% 
  crop(vect(sjv_boundary))

plot(gkr_habitat_masked_sjv, 
     main = "GKR baseline habitat suitability prediction (SJV extent)")
```

```{r}
# plot at the full extent and add sjv boundary shapefile as red line
tm_shape(gkr_habitat_masked) +
  tm_raster(title = "Habitat suitability", 
            palette = "viridis", 
            style = "cont") +
  tm_shape(sjv_boundary) +
  tm_borders(col = "red", lwd = 2) + 
  tm_graticules(lines = FALSE) +
  tm_title("GKR baseline habitat suitability")
```
