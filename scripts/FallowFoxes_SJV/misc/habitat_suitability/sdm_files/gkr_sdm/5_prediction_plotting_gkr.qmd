...
# transform basins
sjv_basins <- st_transform(sjv_basins, crs = crs(gkr_baseline))
crs(sjv_basins)
# convert to terra vector
...
Calculated in `4_maxent_predictions_gkr.qmd`, reclassify continuous habitat suitability layers into binary suitable/unsuitable layers using the threshold that maximizes true positive/negative rate (i.e., specificity + sensitivity). This is **0.47** for the GKR maxent model.

```{r}
# define the reclass matrix
reclass_matrix_future <- matrix(c(0, 0.47, 0,
                                  0.47, 1, 1),
                                ncol = 3, byrow = TRUE)
...
gkr_rcp45_2049_basin <- gkr_rcp45_2049 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
gkr_rcp45_2069_basin <- gkr_rcp45_2069 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
gkr_rcp85_2049_basin <- gkr_rcp85_2049 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
gkr_rcp85_2069_basin <- gkr_rcp85_2069 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)

# stack the future predictions and name them
future_stack <- c(gkr_rcp45_2049_basin, gkr_rcp45_2069_basin, 
                  gkr_rcp85_2049_basin, gkr_rcp85_2069_basin)
names(future_stack) <- c("RCP 4.5 (2020-2049)", "RCP 4.5 (2040-2069)", 
                         "RCP 8.5 (2020-2049)", "RCP 8.5 (2040-2069)")

# plot the future predictions
gkr_fut_map <- tm_shape(future_stack, bbox = basin_bbox_expanded) +
...
gkr_baseline_basin <- gkr_baseline %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)

# plot baseline predictions
gkr_baseline_map <- tm_shape(gkr_baseline_basin, bbox = basin_bbox_expanded) +
