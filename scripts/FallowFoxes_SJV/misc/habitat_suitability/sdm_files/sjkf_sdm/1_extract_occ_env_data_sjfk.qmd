---
title: "Extract environmental data to SJKF occurances"
author: "Stella Wing"
format: html
editor: visual
date: last-modified
...
This script acquires San Joaquin Kit Fox (SJKF) occurrence points from GBIF using the `rgbif` package, and then extracts environmental data to the SJKF occurrence points for use in MaxEnt modeling. The environmental data includes:
...
```{r}
# QC: checking occ points locations 
...
cell_ids <- terra::cellFromXY(env_rasters[[1]], crds(sjkf_occ_vect))

# shuffle points so the "kept" point per cell is random
set.seed(123)
random_order <- sample(seq_len(nrow(sjkf_occ_vect)))
sjkf_occ_shuffled <- sjkf_occ_vect[random_order, ]
cell_ids_shuffled <- cell_ids[random_order]

# keep first (now random) point per cell
sjkf_thinned_vect <- sjkf_occ_shuffled[!duplicated(cell_ids_shuffled), ]
crs(sjkf_thinned_vect)
...
write_csv(sjkf_occ_env_df, here("data/intermediate/misc/habitat_suitability/sdm_files/sjkf_sdm/1_extract_occ_env_data_sjkf/sjkf_occ_env_data.csv"))
```
