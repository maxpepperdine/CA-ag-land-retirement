...
```{r}
# baseline sjkf model
sjkf_baseline <- rast(here("data/4_maxent_predictions/maxent_sjkf_pred_masked_lakes.tif"))

# load SJV boundary for masking
sjv_boundary <- read_sf(here("data/sjv_counties/sjv_counties.shp"))

# load CA boundary for inset map
ca_boundary <- read_sf(here("data/ca_state/ca_state_boundary_wgs84.shp"))
```
...
```{r}
# RCP 4.5 (2020-2049)
sjkf_rcp45_2049 <- rast(here("data/4_maxent_predictions/maxent_sjkf_pred_rcp45_2020_2049_masked_lakes.tif"))

# RCP 4.5 (2040-2069)
sjkf_rcp45_2069 <- rast(here("data/4_maxent_predictions/maxent_sjkf_pred_rcp45_2040_2069_masked_lakes.tif"))

# RCP 8.5 (2020-2049)
sjkf_rcp85_2049 <- rast(here("data/4_maxent_predictions/maxent_sjkf_pred_rcp85_2020_2049_masked_lakes.tif"))

# RCP 8.5 (2040-2069)
sjkf_rcp85_2069 <- rast(here("data/4_maxent_predictions/maxent_sjkf_pred_rcp85_2040_2069_masked_lakes.tif"))
```
...
Calculated in `4_maxent_predictions_sjkf.qmd`, reclassify continuous habitat suitability layers into binary suitable/unsuitable layers using the threshold that maximizes true positive/negative rate (i.e., specificity + sensitivity). This is **0.31** for the sjkf maxent model.

```{r}
# define the reclass matrix
reclass_matrix_future <- matrix(c(0, 0.31, 0,
                                  0.31, 1, 1),
                                ncol = 3, byrow = TRUE)
...
print(inset_map, vp = grid::viewport(x = 0.70, y = 0.5, width = 0.2, height = 0.25))
```
...
future_stack <- c(sjkf_rcp45_2049_sjv, sjkf_rcp45_2069_sjv, 
                  sjkf_rcp85_2049_sjv, sjkf_rcp85_2069_sjv)
names(future_stack) <- c("RCP 4.5 (2020-2049)", "RCP 4.5 (2040-2069)", 
                         "RCP 8.5 (2020-2049)", "RCP 8.5 (2040-2069)")

# plot the future predictions
sjkf_fut_map <- tm_shape(future_stack, bbox = sjv_bbox_expanded) +
...
# add inset using viewport
print(inset_map, vp = grid::viewport(x = 0.70, y = 0.3, width = 0.15, height = 0.2))
```
...
sjkf_baseline_map <- tm_shape(sjkf_baseline_sjv, bbox = sjv_bbox_expanded) +
...
# add inset using viewport
print(inset_map, vp = grid::viewport(x = 0.75, y = 0.2, width = 0.2, height = 0.25))
```
