---
title: "SJKF habitat suitability prediction plotting & comparison"
author: "Stella Wing"
format: html
editor: visual
editor_options: 
...
This script generates plots and difference maps comparing the baseline sjkf habitat suitability model with future predictions under various emissions scenarios.
...
sjv_boundary <- st_transform(sjv_boundary, crs = crs(sjkf_baseline))
crs(sjv_boundary)
# convert to terra vector
sjv_vect <- vect(sjv_boundary)

# transform CA boundary to EPSG:3310
ca_boundary <- st_transform(ca_boundary, crs = crs(sjkf_baseline))
crs(ca_boundary)
# convert to terra vector
ca_vect <- vect(ca_boundary)

# transform basins
sjv_basins <- st_transform(sjv_basins, crs = crs(sjkf_baseline))
crs(sjv_basins)
# convert to terra vector
basins_vect <- vect(sjv_basins)
# merge to one polygon for clipping
basins_union <- st_union(sjv_basins)
basins_union_vect <- vect(basins_union)
```
...
cont_diff_rcp45_2049 <- sjkf_rcp45_2049 - sjkf_baseline
cont_diff_rcp45_2069 <- sjkf_rcp45_2069 - sjkf_baseline
cont_diff_rcp85_2049 <- sjkf_rcp85_2049 - sjkf_baseline
cont_diff_rcp85_2069 <- sjkf_rcp85_2069 - sjkf_baseline

# clip to SJV boundary
cont_diff_rcp45_2049_basin <- cont_diff_rcp45_2049 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
cont_diff_rcp45_2069_basin <- cont_diff_rcp45_2069 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
cont_diff_rcp85_2049_basin <- cont_diff_rcp85_2049 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
cont_diff_rcp85_2069_basin <- cont_diff_rcp85_2069 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
```
...
Calculated in `4_maxent_predictions_sjkf.qmd`, reclassify continuous habitat suitability layers into binary suitable/unsuitable layers using the threshold that maximizes true positive/negative rate (i.e., specificity + sensitivity). This is **0.38** for the sjkf maxent model.

```{r}
# define the reclass matrix
reclass_matrix_future <- matrix(c(0, 0.38, 0,
                                  0.38, 1, 1),
                                ncol = 3, byrow = TRUE)

# reclassify baseline
sjkf_baseline_bin <- classify(sjkf_baseline, reclass_matrix_future)

# reclassify future predictions
sjkf_rcp45_2049_bin <- classify(sjkf_rcp45_2049, reclass_matrix_future)
sjkf_rcp45_2069_bin <- classify(sjkf_rcp45_2069, reclass_matrix_future)
sjkf_rcp85_2049_bin <- classify(sjkf_rcp85_2049, reclass_matrix_future)
sjkf_rcp85_2069_bin <- classify(sjkf_rcp85_2069, reclass_matrix_future)
...
combined_rcp45_2049 <- sjkf_baseline_bin * 10 + sjkf_rcp45_2049_bin
combined_rcp45_2069 <- sjkf_baseline_bin * 10 + sjkf_rcp45_2069_bin
combined_rcp85_2049 <- sjkf_baseline_bin * 10 + sjkf_rcp85_2049_bin
combined_rcp85_2069 <- sjkf_baseline_bin * 10 + sjkf_rcp85_2069_bin
...
bin_diff_rcp45_2049_basin <- bin_diff_rcp45_2049 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
bin_diff_rcp45_2069_basin <- bin_diff_rcp45_2069 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
bin_diff_rcp85_2049_basin <- bin_diff_rcp85_2049 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
bin_diff_rcp85_2069_basin <- bin_diff_rcp85_2069 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
```
...
basin_sf <- st_as_sf(basins_union_vect)
ca_sf <- st_as_sf(ca_vect)

## Create expanded bounding box for basins
basin_bbox <- st_bbox(basin_sf)
basin_buffer <- 0.05
x_expand <- basin_buffer * (unname(basin_bbox["xmax"]) - unname(basin_bbox["xmin"]))
y_expand <- basin_buffer * (unname(basin_bbox["ymax"]) - unname(basin_bbox["ymin"]))

basin_bbox_expanded <- st_bbox(c(
  xmin = unname(basin_bbox["xmin"]) - x_expand,
  xmax = unname(basin_bbox["xmax"]) + x_expand,
  ymin = unname(basin_bbox["ymin"]) - y_expand,
  ymax = unname(basin_bbox["ymax"]) + y_expand
), crs = st_crs(basin_sf))
...
bin_diff_stack <- c(bin_diff_rcp45_2049_basin, bin_diff_rcp45_2069_basin, 
                    bin_diff_rcp85_2049_basin, bin_diff_rcp85_2069_basin)
...
main_map <- tm_shape(bin_diff_stack, bbox = basin_bbox_expanded) +
  ...
  tm_shape(basins_union_vect) +
  tm_borders(col = "grey25", lwd = 0.1)

# create the inset map
inset_map <- tm_shape(ca_vect, bbox = ca_bbox_expanded) +
  tm_borders(col = "grey25", lwd=1) +
  tm_shape(basins_union_vect) +
...
print(inset_map, vp = grid::viewport(x = 0.63, y = 0.5, width = 0.2, height = 0.25))
```
...
cont_diff_stack <- c(cont_diff_rcp45_2049_basin, cont_diff_rcp45_2069_basin, 
                     cont_diff_rcp85_2049_basin, cont_diff_rcp85_2069_basin)
...
# add inset using viewport
print(inset_map, vp = grid::viewport(x = 0.78, y = 0.3, width = 0.2, height = 0.25))
```
...
sjkf_rcp45_2049_basin <- sjkf_rcp45_2049 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
sjkf_rcp45_2069_basin <- sjkf_rcp45_2069 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
sjkf_rcp85_2049_basin <- sjkf_rcp85_2049 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)
sjkf_rcp85_2069_basin <- sjkf_rcp85_2069 %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)

# stack the future predictions and name them
future_stack <- c(sjkf_rcp45_2049_basin, sjkf_rcp45_2069_basin, 
                  sjkf_rcp85_2049_basin, sjkf_rcp85_2069_basin)
names(future_stack) <- c("RCP 4.5 (2020-2049)", "RCP 4.5 (2040-2069)", 
                         "RCP 8.5 (2020-2049)", "RCP 8.5 (2040-2069)")

# plot the future predictions
sjkf_fut_map <- tm_shape(future_stack, bbox = basin_bbox_expanded) +
...
# add inset using viewport
print(inset_map, vp = grid::viewport(x = 0.65, y = 0.3, width = 0.2, height = 0.25))
```
...
sjkf_baseline_basin <- sjkf_baseline %>% 
  mask(basins_union_vect) %>% 
  crop(basins_union_vect)

# plot baseline predictions
sjkf_baseline_map <- tm_shape(sjkf_baseline_basin, bbox = basin_bbox_expanded) +
  ...
  tm_raster(col.scale = tm_scale_continuous(values = "viridis",
            ...
            col.legend = tm_legend(title = "Habitat suitability"), 
            col.free = FALSE) +
  tm_shape(basins_vect) +
  tm_borders(col = "red4", lwd = 0.1)

# plot main map
print(sjkf_baseline_map)

# add inset using viewport
print(inset_map, vp = grid::viewport(x = 0.68, y = 0.20, width = 0.2, height = 0.25))
```
