...
#### Create 100 km buffer around SJKF occ points & then clip to SJV boundary 
...
sjkf_buffer <- st_buffer(sjkf_sf, dist = 100000)
...
# clip to the SJV
# read SJV shapefile
sjv <- read_sf(here("data/sjv_counties/sjv_counties.shp"))

# make sure CRS matches
sjv <- st_transform(sjv, st_crs(vertebrate_unique))

# keep only points within SJV
vertebrate_sjv <- st_filter(vertebrate_unique, sjv)


# extract coordinates for MaxEnt and env data extraction
background_coords <- st_coordinates(vertebrate_sjv)
colnames(background_coords) <- c("longitude", "latitude")

# Create final dataframe
background_df <- cbind(
  as.data.frame(st_drop_geometry(vertebrate_sjv)),
  background_coords
)
```
...
```{r}
# save SF to test
write_sf(vertebrate_sjv, here("data/sjkf_bg_points.shp"),
         append = FALSE)
