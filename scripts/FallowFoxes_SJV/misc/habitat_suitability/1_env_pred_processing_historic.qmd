...
## PET
pet_rast <- rast(here("data/raw/bcm/BCMv8_1991_2020_aves/pet1991_2020_ave.asc"))
crs(pet_rast) <- "EPSG:3310" # assign correct CRS
# QC resolution & crs
res(pet_rast) # 270m resolution
crs(pet_rast) # EPSG:3310
...
# crop PET raster to CA boundary
pet_ca <- pet_rast %>%
  crop(ca_state) %>%
  mask(ca_state)

# view range of PET values in CA
summary(values(pet_ca))
# -9999 indicates no data values -- these should be NA
values(pet_ca)[values(pet_ca) == -9999] <- NA
...
plot(pet_ca, main = "PET (CA Boundary)")
...
## PET -----------------------------------------------
# apply function to PET raster
pet_ca_filled <- fill_all_na(pet_ca)

# crop to PPT to ensure no edge effects
pet_ca_filled <- pet_ca_filled %>%
  crop(ppt_ca) %>%
  mask(ppt_ca)

# resample pet to match aet extent
pet_ca_filled <- resample(pet_ca_filled, 
                          aet_ca_filled, 
                          method = "mean")
...
# plot final AET, PET & CWD raster
plot(aet_ca_filled, main = "Mean AET (filled NA values)")
plot(pet_ca_filled, main = "Mean PET (filled NA values)")
...
# export PET raster
writeRaster(pet_ca_filled,
            filename = file.path(output_dir, "pet1991_2020_ave_CA_270m.tif"), 
            overwrite = TRUE)
