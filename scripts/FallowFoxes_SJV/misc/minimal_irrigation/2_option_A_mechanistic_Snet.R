# ==============================================================================
# PATH A: MECHANISTIC ETc APPROACH — S_net Calculator
# ==============================================================================
# GOAL: Project future water use by computing future ETc from baseline ETc
#       scaled by PET change, then derive applied water using baseline
...
#   Path A scales ETc and AW independently via IE, so crops with different
#   efficiencies will see different AW responses to the same ETc change.
#   An inefficient crop (low IE) will see AW amplified more than an efficient
#   one. Path B scales AW directly by the PET ratio, ignoring IE.
#
# DATA SOURCES:
...
# Output directory
output_dir <- "data/intermediate/misc/minimal_irrigation/2_option_A_mechanistic_Snet"
...
# Write GeoPackage
output_path <- file.path(output_dir, "Snet_PathA_Mechanistic.gpkg")
st_write(prioritizr_final, output_path, delete_dsn = TRUE)
cat("Written to:", output_path, "\n")

# Also write a CSV (without geometry) for quick inspection
csv_path <- file.path(output_dir, "Snet_PathA_Mechanistic.csv")
prioritizr_final %>%
  st_drop_geometry() %>%
  write_csv(csv_path)
...
# # Also write a CSV (without geometry and only 1000 rows) for quick inspection
# csv_sample_path <- file.path(output_dir, "Snet_PathA_Mechanistic_sample.csv")
# prioritizr_final %>%
#   st_drop_geometry() %>%
#   slice_sample(n = 1000) %>%
#   write_csv(csv_sample_path)
