# =============================================================================
# PRIORITIZR COMBINED OPTIMIZATION WITH PROTECTED AREAS —
# RESULTS VISUALIZATIONS
# =============================================================================
# Purpose: Generate figures from the combined water + habitat prioritizr
#          results (with locked-in PAs) produced by
#          9_3_prioritizr_combined_PAs.R.
#
# FIGURES:
#   1. Scenario comparison grid: Acres, Revenue, Water Savings (6 scenarios)
...
#   5. Cost premium: combined vs. water-only vs. habitat-only-with-PAs
#
# INPUTS:
#   - prioritizr_combined_PAs_results.RData       (from 9_3_*_PAs.R)
#   - prioritizr_cross_temporal_PAs_results.RData (from 9_1_*_PAs.R)
#   - valley_wide_summary.csv                     (from 9_2)
#
# OUTPUTS:
...
# --- Diagnostics ---
cat("Loaded combined prioritizr results (with PAs):\n")
...
fig2 <- ggplot(heatmap_data, aes(x = scenario_label, y = feature_label, fill = pct_achieved)) +
  ...
  labs(
    title = "Target achievement across all combined scenarios (with locked-in PAs)",
...
fig4 <- map_freq_suit + map_freq_hq +
  ...
  plot_annotation(
    title = "Selection frequency across water scenarios (combined optimization, with locked-in PAs)",
...
fig5 <- (fig5a + fig5b) / (fig5c_panel + fig5d) +
  plot_annotation(
    title = "Cost of co-optimization: combined vs. individual objective solutions (with locked-in PAs)",
    subtitle = "Comparing water-only, habitat-only-with-PAs, and combined-with-PAs optimization under Baseline and RCP 4.5 scenarios",
    theme = theme(
      plot.title    = element_text(face = "bold", size = 13),
      plot.subtitle = element_text(color = "gray40", size = 10)
    )
...
fig5b_full <- ggplot(fig5_full_data, aes(x = water_label_clean, y = total_cost / 1e6,
  ...
  labs(
    ...
    subtitle = "Combined-with-PAs optimization costs vs. water-only baseline for each climate scenario",
