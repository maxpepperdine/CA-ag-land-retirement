...
#          targets while minimizing foregone agricultural revenue, with
...
#      PAs (AW version). Applied to all 6 problems.
# =============================================================================
...
#   - AW      = 0 for every water feature column (no contribution to targets)
...
# Water columns: raw AF columns for both S_net and AW (plus ETc if present).
# We zero out all of them on PAs so that any downstream code referencing
# these columns sees PA contributions as 0.
...
#
# PET ratios are computed using FIELD planning units only (PAs excluded —
# they have no agricultural water use and would bias the weighted mean if
# included).
...
# --- Derive RCP8.5 scaling factor from BCM PET data (FIELDS ONLY) ---
...
blm_plot <- ggplot(blm_results, aes(x = total_cost, y = boundary)) +
  ...
  labs(
    title    = "BLM Calibration: Cost vs. Boundary Length (AW version, w/ locked-in PAs)",
...
for (i in 1:nrow(scenarios)) {
  ...
  # Print summary
  cat(sprintf("    Selected (new): %d fields | Cost: $%s | Acres: %s | AW: %s AF\n",
              nrow(sel),
              format(round(sum(sel$revenue, na.rm = TRUE)), big.mark = ","),
...
save(
  ...
  file = file.path(output_dir, "prioritizr_combined_AW_PAs_results.RData")
)
...
write_csv(summary_all,
          file.path(output_dir, "combined_AW_PAs_solution_comparison.csv"))
write_csv(target_detail,
          file.path(output_dir, "combined_AW_PAs_target_detail.csv"))
write_csv(blm_results,
          file.path(output_dir, "combined_AW_PAs_blm_calibration.csv"))
